...
def preprocess():
    """ Input:
     ...
     - feature selection"""
    # default threhsold for variance is greater than 0
    ...
    # clean = feature_select(data=mat[1])
    # dataset = clean,mat[-1]
    ...
    # Feature selection
    features = feature_selection(data=train_data, threshold=0.0)
    train_data = train_data[:, features]
    test_data = test_data[:, features]
    validation_data = validation_data[:, features]

    # one-hot
    train_hot = np.array([one_heater(int(i)) for i in train_label])
    test_hot = np.array([one_heater(int(j)) for j in test_label])
    vali_hot = np.array([one_heater(int(k)) for k in validation_label])
    print("Preprocessing done!")
    return train_data, train_label, train_hot, validation_data, validation_label, vali_hot, test_data, test_label, test_hot, features
...
train_data, train_label, train_hot, validation_data, validation_label, vali_hot, test_data, test_label, test_hot, features = preprocess();
...
# lambdaval = np.float64(sys.argv[2]);
...
print(n_input, n_hidden, n_class, train_data.shape, train_label.shape, train_hot.shape, lambdaval)
